...
function getIcon(name: string): React.ReactNode {
  return iconMap[name] ?? iconMap.activity
}

// Convert NavConfig to NavItem with resolved icons and client-scoped paths
function buildClientNav(configs: NavConfig[], clientId: string | null): NavItem[] {
  return configs.map((c) => ({
    label: c.label,
    href: c.href === '/' || c.href === '/settings' || !clientId ? c.href : `/clients/${clientId}${c.href}`,
    icon: getIcon(c.icon),
  }))
}
...
export default function Sidebar({ role, clientId, userEmail, displayName, productsEnabled }: SidebarProps) {
  const router = useRouter()
  const pathname = usePathname()
  const [isOpen, setIsOpen] = useState(false)

  const navItems = buildClientNav(getClientNav(productsEnabled), clientId)
  ...
  const sidebarContent = (
    <>
      ...
      <div className="px-6 py-5 border-b border-gray-800">
        ...
        <p className="text-gray-400 text-xs mt-0.5">Business Dashboard</p>
      </div>
